    ...
    <div className="min-h-screen bg-cream flex flex-col justify-center items-center p-6 md:p-12 font-sans">
      <div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
        ...
        <div className="md:col-span-5 space-y-6 text-left">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-charcoal font-heading leading-tight">
            DealFlow360
          </h1>
          ...
          <div className="space-y-4 pt-2">
            <div className="flex items-start gap-3 text-sm md:text-base text-muted">
              <Shield className="w-5 h-5 text-charcoal shrink-0 mt-0.5" />
              <span>Self-service customer negotiation with instant margin feedback.</span>
            </div>
            <div className="flex items-start gap-3 text-sm md:text-base text-muted">
              <Sparkles className="w-5 h-5 text-charcoal shrink-0 mt-0.5" />
              <span>Automated multi-warehouse split engine with live freight optimization.</span>
            </div>
            <div className="flex items-start gap-3 text-sm md:text-base text-muted">
              <UserCheck className="w-5 h-5 text-charcoal shrink-0 mt-0.5" />
        ...
        <div className="md:col-span-7">
          <div className="card bg-white border border-warm p-8 md:p-10 shadow-xl rounded-2xl space-y-6">
            
            {/* Auth Tab Selector */}
            <div className="flex border-b border-warm text-base font-semibold">
              <button
                onClick={() => setActiveTab('login')}
                className={`pb-3 px-6 flex-1 text-center transition-all ${
                  activeTab === 'login'
                    ? 'border-b-2 border-charcoal text-charcoal font-bold text-lg'
            ...
            {activeTab === 'login' && (
              <form onSubmit={handleLogin} className="space-y-5 text-left">
                ...
                <button type="submit" className="btn btn-primary w-full py-3.5 text-base font-semibold flex items-center justify-center gap-2.5 shadow-md mt-2">
